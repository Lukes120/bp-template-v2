...
function bp_db_migrate(PDO $pdo): void {
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS utenti (
            ...
            ruolo         TEXT NOT NULL CHECK(ruolo IN ('admin','supervisore','user')),
            email         TEXT,
            odoo_uid      INTEGER,
...
function bp_session_create(string $userId, int $ttlSeconds = 604800): string {
...
function bp_offerta_upsert(array $o, ?array $actor = null): void {
    ...
    foreach (['id','userId','userName','nome','cliente','tipo','nOrdineOdoo','data','note','speseGenerali','scontoStato','scontoTipo','scontoValore','scontoNota','allegataOdoo','prezzoImpostoAttivo','prezzoImpostoValore'] as $k) {
        unset($payload[$k]);
    }
    ...
    if ($isUpdate) {
        ...
        $stmt = bp_db()->prepare("
            INSERT INTO offerte (id, user_id, user_name, nome, cliente, tipo, n_ordine_odoo, data,
                ...
                prezzo_imposto_attivo, prezzo_imposto_valore, payload_json)
